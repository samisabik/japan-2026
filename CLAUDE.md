...
- `app.js`: one IIFE containing the tabs (URL hash + localStorage), the Leaflet map (created the first time the map tab opens), category filter chips, live location via `watchPosition`, and the "Closest to you" card.
...
Test at a 412×915 mobile viewport. Check that nothing scrolls sideways, the bottom bar doesn't cover content, the map fills the space above the bar, and location works when served over `http://localhost`.
